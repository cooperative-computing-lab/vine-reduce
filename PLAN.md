...
│                                                                              │
│ executor_wrapper(chunk, metadata)                                            │
│   │                                                                          │
│   ▼                                                                          │
│ chunk_to_args(chunk, dataset_metadata, distributor_metadata) ──► args        │
│   │                                                                          │
│   ▼                                                                          │
│ executor.submit(processor, args, metadata=).result()                         │
...
- **executor** (remote object): an `Executor` protocol instance (`submit`/`shutdown`, named
  ...
  `executor_wrapper` calls `executor.submit(processor, args,
  metadata={"dataset": ..., "distributor": ..., "executor": ...}).result()`, then shuts the
  (freshly deserialized, per-call) instance down. Default: `SimpleExecutor()` - see "Executors and remote
  environments".
...
metadata = resources(kind): a default resource dict (e.g. {"cores": ...}) for calls of this kind,
    ...
    processor submission and passes the result as chunk_to_args's distributor_metadata argument
    (and, via executor_wrapper, as metadata["distributor"]) - see "Data Flow". It is a fallback default only: the execution
