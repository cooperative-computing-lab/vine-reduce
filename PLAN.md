...
result_id = submit(priority, category, kind, executor_wrapper | reducer_wrapper, *args,
    ...
    requests to each. is_checkpoint marks a call whose result must be durable (a non-final checkpoint
    or a final result - see "Temporary Results, Checkpoints, and Restart" above); a distributor that
    distinguishes durable from disposable storage uses it to pick which at submit time (e.g.
    TaskVineDistributor's vine_file(cache=True) vs vine_temp()). Returns a result_id, used later to
...
path = checkpoint_path(result_id): local, durable on-disk path for a completed (Success) result_id
    ...
    itself (e.g. TaskVineDistributor's own checkpoint_dir constructor argument) - vine_reduce calls
    this for a non-final checkpoint to learn where it ended up, to record in the checkpoint db.
