...
def main():
    ...
    with workers, distributor:
        ...
        # final results (results_dir) and intermediate checkpoints
        # (checkpoint_dir - VineReduce owns this path and hands it to
        # distributor itself), and which executor backend runs the actual
        # tasks (distributor). reducer and is_result are left at their
        # defaults: plain `a += b` addition, and "a group is final once it
        # covers every entry of the dataset" (see defaults.py for both).
        vr = VineReduce(
            processors={
